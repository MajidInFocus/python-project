...
class TrainTicketBookingSystem:
    def __init__(self, root):
        ...
        self.root.bind("<Configure>", self.load_and_display_image)

        # Main window buttons
        user_button = ttk.Button(self.root, text="User", command=self.open_user_window)
        user_button.pack(pady=20)

        admin_button = ttk.Button(self.root, text="Admin", command=self.open_admin_window)
...
# Load the original image without resizing
    def load_image(self):
        try:
            original_image = Image.open("/Users/majid/Desktop/UIgroupProject/projectGUI/background.png")
            self.display_image = ImageTk.PhotoImage(original_image)

            # Create a canvas to display the image
            self.canvas = tk.Canvas(self.root, width=original_image.width, height=original_image.height)
            self.canvas.pack()
...
class AdminWindow:
    ...
    def display_screen(self):
        ...
        revenues = [0.00, 0.00, 0.00, 0.00]  # Placeholder values
...
class TrainTicketBookingSystem:
    def __init__(self, root):
        ...
        # Main window buttons
        user_button = ttk.Button(self.root, text="User", command=self.open_user_window, style="TButton")
        user_button.pack(pady=20)

        admin_button = ttk.Button(self.root, text="Admin", command=self.open_admin_window, style="TButton")
        admin_button.pack(pady=20)

        close_button = ttk.Button(self.root, text="Close", command=self.root.destroy, style="TButton")
        close_button.pack(pady=20)
